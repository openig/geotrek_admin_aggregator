API_BASE_URL = 'https://geotrek-admin.cevennes-parcnational.net/api/v2/'
# URL de l'API v2 de la base de données source
PORTALS = ['DEP_48']  # Si nécessaire, nom du (ou des) portail(s) de la base
# source dont on veut récupérer les données.
AUTHENT_STRUCTURE = 'PNC'  # Nom de la source des données.
# Doit correspondre à une entrée dans la table "authent_structure".
# Indispensable pour la tracabilité des données.
AUTH_USER = 'gadmin48'  # Nom de l'user auquel sera attribuée la création des médias
